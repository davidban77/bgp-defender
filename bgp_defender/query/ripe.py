...
class RipeConnector:
    ...
    def search_asn(self, asn):
        """
        Testing searching for asn
        """
        _url = '{}/search?source=ripe&query-string=google&type-filter=inetnum'.format(
            self.base_url)
        _response = self.http_call('get', _url)
        ...
        print('[INFO] Prefix found - ASN {}'.format(asn))
...
if __name__ == "__main__":
    ...
    prefix = test_session.search_asn('algo')
    if prefix:
        pprint(prefix)
